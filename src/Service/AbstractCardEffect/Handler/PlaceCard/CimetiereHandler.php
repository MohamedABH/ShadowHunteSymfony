...
class CimetiereHandler implements AbstractCardEffectHandlerInterface
{
    ...
    {
        return $card instanceof PlaceCard && $card->getName() === 'Cimetière';
    }
    ...
    public function execute(AbstractCard $card, Player $player, Game $game, array $context = []): AbstractCardEffectResult
    {
        if (!$this->supports($card)) {
            return AbstractCardEffectResult::failure('Unsupported card type for CimetiereHandler');
        }

        $drawnLocation = $this->deckService->drawCardFromDeck($game, $player, LocationEnum::DARK_DECK);
        if (!$drawnLocation || !$drawnLocation->getActionCard()) {
            return AbstractCardEffectResult::failure('No Dark card available to draw.');
        }
        ...
        if (!empty($missingContext)) {
            return AbstractCardEffectResult::requiresAction(
                sprintf('Dark card "%s" drawn. Additional information is required to execute it.', $drawnCard->getName()),
                [
                    'drawnCard' => [
                        'id' => $drawnCard->getId(),
        ...
        if ($drawnCardResult->hasPendingActions()) {
            return AbstractCardEffectResult::requiresAction(
                sprintf('Dark card "%s" drawn. Follow-up action required.', $drawnCard->getName()),
        ...
        return AbstractCardEffectResult::success(
            sprintf('%s drew and executed dark card "%s"', $player->getUser()->getUsername(), $drawnCard->getName()),
            [
                'player_id' => $player->getId(),
                'effect' => 'draw_and_execute_dark_card',
                'drawnCard' => [
                    'id' => $drawnCard->getId(),
                    'name' => $drawnCard->getName(),
                ],
