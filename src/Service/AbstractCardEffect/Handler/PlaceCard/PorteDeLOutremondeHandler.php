...
class PorteDeLOutremondeHandler implements AbstractCardEffectHandlerInterface
{
    ...
    {
        return $card instanceof PlaceCard && $card->getName() === 'Porte de l\'Outremonde';
    }
    ...
    public function execute(AbstractCard $card, Player $player, Game $game, array $context = []): AbstractCardEffectResult
    {
        if (!$this->supports($card)) {
            return AbstractCardEffectResult::failure('Unsupported card type for PorteDeLOutremondeHandler');
        }

        $deckType = strtolower((string) ($context['deckType'] ?? ''));
        $deckLocation = match ($deckType) {
        ...
        if (!$deckLocation) {
            return AbstractCardEffectResult::requiresAction(
                'Please choose a deck to draw from.',
                ['required' => ['deckType'], 'allowed' => ['dark', 'light', 'sight']]
            );
        }

        $drawnLocation = $this->deckService->drawCardFromDeck($game, $player, $deckLocation);
        if (!$drawnLocation || !$drawnLocation->getActionCard()) {
            return AbstractCardEffectResult::failure(sprintf('No %s card available to draw.', ucfirst($deckType)));
        }
        ...
        if (!empty($missingContext)) {
            return AbstractCardEffectResult::requiresAction(
                sprintf('%s card "%s" drawn. Additional information is required to execute it.', ucfirst($deckType), $drawnCard->getName()),
                [
                    'drawnCard' => [
                        'id' => $drawnCard->getId(),
        ...
        if ($drawnCardResult->hasPendingActions()) {
            return AbstractCardEffectResult::requiresAction(
                sprintf('%s card "%s" drawn. Follow-up action required.', ucfirst($deckType), $drawnCard->getName()),
        ...
        return AbstractCardEffectResult::success(
            sprintf('%s drew and executed %s card "%s"', $player->getUser()->getUsername(), $deckType, $drawnCard->getName()),
            [
                'player_id' => $player->getId(),
                'effect' => sprintf('draw_and_execute_%s_card', $deckType),
                'deckType' => $deckType,
                'drawnCard' => [
                    'id' => $drawnCard->getId(),
                    'name' => $drawnCard->getName(),
