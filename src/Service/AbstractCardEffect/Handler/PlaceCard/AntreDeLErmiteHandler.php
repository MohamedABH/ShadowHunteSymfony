...
/**
 * Handler for "Antre de l'ermite" card
 * Effect: You can draw a Sight card.
 */
#[AutoconfigureTag('app.abstract_card_effect_handler')]
class AntreDeLErmiteHandler implements AbstractCardEffectHandlerInterface
{
    ...
    {
        return $card instanceof PlaceCard && $card->getName() === 'Antre de l\'ermite';
    }
    ...
    public function execute(AbstractCard $card, Player $player, Game $game, array $context = []): AbstractCardEffectResult
    {
        if (!$this->supports($card)) {
            return AbstractCardEffectResult::failure('Unsupported card type for AntreDeLErmiteHandler');
        }

        $drawnLocation = $this->deckService->drawCardFromDeck($game, $player, LocationEnum::SIGHT_DECK);
        if (!$drawnLocation || !$drawnLocation->getActionCard()) {
            return AbstractCardEffectResult::failure('No Sight card available to draw.');
        }
        ...
        if (!empty($missingContext)) {
            return AbstractCardEffectResult::requiresAction(
                sprintf('Sight card "%s" drawn. Additional information is required to execute it.', $drawnCard->getName()),
                [
                    'drawnCard' => [
                        'id' => $drawnCard->getId(),
        ...
        if ($drawnCardResult->hasPendingActions()) {
            return AbstractCardEffectResult::requiresAction(
                sprintf('Sight card "%s" drawn. Follow-up action required.', $drawnCard->getName()),
        ...
        return AbstractCardEffectResult::success(
            sprintf('%s drew and executed sight card "%s"', $player->getUser()->getUsername(), $drawnCard->getName()),
            [
                'player_id' => $player->getId(),
                'effect' => 'draw_and_execute_sight_card',
                'drawnCard' => [
                    'id' => $drawnCard->getId(),
                    'name' => $drawnCard->getName(),
                ],
